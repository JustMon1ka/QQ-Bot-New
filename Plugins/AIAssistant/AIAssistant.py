...
#     插件名：AIAssistant \n
#     插件类型：群聊插件 \n
#     插件功能：用户可以通过"<bot name> ask <问题内容>"的形式向远程大模型提问 \n
#     """
...
#         self.introduction = """
#                                 远程大模型问答插件
#                                 用户可以通过"<bot name> ask <问题内容>"的形式向远程大模型提问
#                             """
#         self.init_status()
        
#         # 初始化大模型API配置
#         self.bot_id = '7507558275785998373'  # 默认Coze bot ID
#         self.user_id = '114514'  # 默认用户ID
#         self.api_token = 'pat_FlCKNEJXELg6lbfxRuoNPR5XvAnrzLTVGSOItVwtwZOsKmGBoQm0Fe2UoXwjm2iL'  # API访问令牌
#         self.base_url = 'https://api.coze.cn/open_api/v2'  # Coze API基础URL
...
#         # 检查命令格式
#         if len(command) == 2 and command[1] == 'ask':
...
#         if command[0] != self.bot.bot_name or command[1] != "ask":
#             return
...
#                 message=f"{At(qq=event.user_id)} 请求大模型时出错了: {str(e)}"
#             )
