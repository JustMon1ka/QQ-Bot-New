        ...
        if command_list[1] != command:
            ...
            if group_id not in effected_group:
                ...
                return
                     ...
                     for members in group_memberlist:
                         if legality[f'{members['user_id']}']!=1:
                            message+= self.message_generate(legality=legality,members=members,user_counts_map=user_counts_map)
                        ...
                        for members in group_memberlist:
                            if kicks :
                                if user_counts_map[members['user_id']]==int(self.threshold):
                                    await self.api.groupService.set_group_kick(group_id=group_id,user_id=members['user_id'])
                                    kicks+=1
