    ...
    async def main(self, event, debug):
        ...
        if command_list[1] != command:
            ...
            if group_id not in effected_group:
                ...
                group_member_list = (await self.api.GroupService.get_group_member_list(self, group_id=group_id)).get(
                    "data")
                ...
                for members in group_member_list:
                    ...
                    if members['user_id'] in ignored_ids:
                        continue
                    card_cuts = members['card'].split("-")
                    if len(card_cuts) != 3:
                        legality[f"{members['user_id']}"] = 0
                    else:
                        ...
                        if select_result:
                            ...
                            legality[f"{members['user_id']}"] = -3

                for members in group_member_list:
                    if legality[f"{members['user_id']}"] != 1:
                ...
                try:
                    ...
                    for members in group_member_list:
                        if kicks:
                            if int(user_counts_map[members['user_id']]) == int(self.threshold):
                                # await self.api.groupService.set_group_kick(group_id=group_id,
